...
public class XListView extends ListView {
	...
	// 是显示加载更多 还是 已加载完毕
	private boolean isfootershowtxet;
	...
	public void setAdapter(ListAdapter adapter) {
		// 确保footer最后添加并且只添加一次super(context, attrs)
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		totalItemCount = getAdapter() != null ? getAdapter().getCount() : 0;
		switch (ev.getAction()) {
	...
	public void setStateNotData(boolean enable) {
		isfootershowtxet = enable;
		footerView.show();
		if (!isfootershowtxet) {
			footerView.setOnClickListener(null);
			footerView.setState(ZListViewFooter.STATE_NULLDATA);
		} else {
			isfootershowtxet = false;
			footerView.setState(ZListViewFooter.STATE_NORMAL);
		}
	...
	public void setNoData() {
		footerView.show();
		footerView.setOnClickListener(null);
		footerView.setState(ZListViewFooter.STATE_NULL_DATA);
	}
