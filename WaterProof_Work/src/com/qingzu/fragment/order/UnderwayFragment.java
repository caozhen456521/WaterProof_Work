...
public class UnderwayFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		v = inflater.inflate(R.layout.fragment_underway, container, false);
		initView();
		return v;
	}
	...
	private void initView() {
		...
		mListView = (ZListView) v.findViewById(R.id.fu_listview);
		mListView.setPullLoadEnable(true);

		WorkerListByStateId("2", "1", "10", BIND_ADAPTER);
		...
		mListView.setXListViewListener(new IXListViewListener() {
			...
			public void onRefresh() {
				list.clear();
				WorkerListByStateId("2", "1", "10", REFRESH_DATA_FINISH);
				Count = 1;
			}
			...
			public void onLoadMore() {
				Count = Count + 1;
				CountStr = Count + "";
				WorkerListByStateId("2", CountStr, "10", LOAD_DATA_FINISH);
			}
		});
		...
		myAdapter = new MyAdapter<OrderListModel>(getActivity(), list,
			...
			public void convert(ViewHolder view, final int position,
				...
				if (roleState == 0) {// 老板
					...
					// lwoi_bt.setVisibility(View.VISIBLE);
					// } else
					if (item.getInfoOrder().getStateId() == 2) {
						lwoi_bt2.setVisibility(View.GONE);
