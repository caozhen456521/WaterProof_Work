 ...
 * 
 */
public class YetAffirmFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		v = inflater.inflate(R.layout.fragment_yet_affirm, container, false);
		initView();
		return v;
	}
	...
	private void initView() {
		...
		mListView = (ZListView) v.findViewById(R.id.fya_listview);
		mListView.setPullLoadEnable(true);

		WorkerListByStateId("1", "1", "10", BIND_ADAPTER);
		...
		mListView.setXListViewListener(new IXListViewListener() {
			...
			public void onRefresh() {
				list.clear();
				WorkerListByStateId("1", "1", "10", REFRESH_DATA_FINISH);
				Count = 1;
			}
			...
			public void onLoadMore() {
				Count = Count + 1;
				CountStr = Count + "";
				WorkerListByStateId("1", CountStr, "10", LOAD_DATA_FINISH);
			}
		});
		...
		myAdapter = new MyAdapter<OrderListModel>(getActivity(), list,
			...
			public void convert(ViewHolder view, final int position,
				...
				if (roleState == 0) {// 老板
					...
					// lwoi_bt.setVisibility(View.VISIBLE);
					// } else
					if (item.getInfoOrder().getStateId() == 1) {
					...
					// lwoi_bt.setEnabled(true);
					// }
					// } else if (item.getInfoOrder().getWorkerIsAppraise() ==
					// 0) {
					// lwoi_bt2.setVisibility(View.GONE);
