...
import com.qingzu.fragment.ConversationListFragment;
...
public class ConversationListController implements OnClickListener,
	...
	public ConversationListController(ConversationListView listView,
			ConversationListFragment context, int width) {
	...
	private void initConvListAdapter() {
		...
		mListAdapter = new ConversationListAdapter(mContext.getActivity(),
				mDatas);
		...
		if (position > 0) {
			...
			if (null != conv) {
				...
				if (conv.getType() == ConversationType.group) {
					long groupId = ((GroupInfo) conv.getTargetInfo())
							.getGroupID();
					intent.putExtra(JChatDemoApplication.GROUP_ID, groupId);
					intent.putExtra(JChatDemoApplication.DRAFT, getAdapter()
							.getDraft(conv.getId()));
					intent.setClass(mContext.getActivity(), ChatActivity.class);
					mContext.getActivity().startActivity(intent);
					return;
				} else {
					String targetId = ((UserInfo) conv.getTargetInfo())
							.getUserName();
	...
	public boolean onItemLongClick(AdapterView<?> viewAdapter, View view,
		...
		if (position > 0) {
			...
			if (conv != null) {
				...
				mDialog = DialogCreator.createDelConversationDialog(
						mContext.getActivity(), conv.getTitle(), listener);
				mDialog.show();
