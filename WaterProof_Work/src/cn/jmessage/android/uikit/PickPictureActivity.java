...
public class PickPictureActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (mGroupId != 0) {
			...
			mTargetAppKey = intent
					.getStringExtra(JChatDemoApplication.TARGET_APP_KEY);
	...
	private OnItemClickListener onItemListener = new OnItemClickListener() {
		...
		public void onItemClick(AdapterView<?> viewAdapter, View view,
			...
			if (mIsGroup) {
				...
				intent.putExtra(JChatDemoApplication.TARGET_APP_KEY,
						mTargetAppKey);
			}
			...
			startActivityForResult(intent,
					JChatDemoApplication.REQUEST_CODE_BROWSER_PICTURE);
		}
	};
		...
		public void onClick(View v) {
			...
			case R.id.pick_picture_send_btn:
				...
				if (mPickedList.size() < 1) {
					...
					mDialog.setMessage(PickPictureActivity.this
	...
	private void getThumbnailPictures() {
		...
		for (int i = 0; i < mPickedList.size(); i++) {
			if (BitmapLoader.verifyPictureSize(mPickedList.get(i))) {
				...
				ImageContent.createImageContentAsync(file,
							...
							public void gotResult(int status, String desc,
								...
								if (status == 0) {
									...
									HandleResponseCode.onHandle(
											PickPictureActivity.this, status,
											false);
								}
								mIndex++;
				...
				bitmap = BitmapLoader.getBitmapFromFile(mPickedList.get(i),
						720, 1280);
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		if (resultCode == JChatDemoApplication.RESULT_CODE_SELECT_PICTURE) {
			if (data != null) {
				...
				if (sum > 0) {
					String sendText = PickPictureActivity.this
							.getString(R.string.jmui_send)
							+ "("
							+ sum
							+ "/"
							+ "9)";
					...
					mSendPictureBtn.setText(PickPictureActivity.this
							.getString(R.string.jmui_send));
				}
	...
	private static class MyHandler extends Handler {
		...
		public void handleMessage(android.os.Message msg) {
			...
			if (activity != null) {
				...
				case SEND_PICTURE:
					Intent intent = new Intent();
					intent.putExtra(JChatDemoApplication.MsgIDs,
							activity.mMsgIds);
					activity.setResult(
							JChatDemoApplication.RESULT_CODE_SELECT_ALBUM,
							intent);
